...
public class AlexPlayerController : MonoBehaviour
{
    ...
    public StateMachine<AlexPlayerController> stateMachine;
    ...
    [Range(0, .3f)] public float groundedAccelerationSmoothing = .05f;
    [Range(0, .3f)] public float groundedSlowDownSmoothing = .05f;
    [Range(0, .3f)] public float airborneAccelerationSmoothing = .05f;
    ...
    {
        rb.velocity = Vector3.SmoothDamp(rb.velocity, Vector2.zero, ref zeroVector, groundedSlowDownSmoothing);
    }
    ...
    public void AirStrafe()
    {
        ...
        if (!Physics.Raycast(transform.position, Vector3.right * Mathf.Sign(moveX), airborneWallDetectionDistance, groundLayerMask))
        {
            ...
            rb.velocity = Vector3.SmoothDamp(rb.velocity, targetVelocity, ref zeroVector, airborneAccelerationSmoothing);
        }
...
public class PlayerIdleState : State<AlexPlayerController>
{
    ...
    public override void ExitState(AlexPlayerController owner)
    { }

    public override void FixedUpdateState(AlexPlayerController owner)
    {
        owner.SlowDown();
    }

    public override void UpdateState(AlexPlayerController owner)
    {
...
public class PlayerRunningState : State<AlexPlayerController>
{
    public override void EnterState(AlexPlayerController owner)
    {
        //Debug.Log("shmoovin");
    ...
    public override void FixedUpdateState(AlexPlayerController owner)
    {
        ...
        owner.rb.velocity = Vector3.SmoothDamp(owner.rb.velocity, owner.targetVelocity, ref owner.zeroVector, owner.groundedAccelerationSmoothing);
        #endregion
...
public class PlayerJumpingState : State<AlexPlayerController>
{
    Timer jumpTimer;
    ...
    {
        #region State transition checks
        jumpTimer.DecrementTimer(Time.deltaTime);
        //Collider[] groundCol = Physics.OverlapSphere(owner.groundCheckObject.transform.position, owner.groundCheckRadius, owner.groundLayerMask);
        ...
        //}
        /*else*/
        if (jumpTimer.Done || Input.GetKeyUp(KeyCode.Space))
        {
            owner.stateMachine.ChangeState(owner.playerFallingState);
        }
        #endregion
...
public class PlayerFallingState : State<AlexPlayerController>
{
...
public class PlayerAttackingState : State<AlexPlayerController>
{
    private Timer timer;
    ...
    IEnumerator HitBoxActivationDelay(AlexPlayerController owner)
    {
